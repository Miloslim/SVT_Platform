// Nom du fichier: SequenceForm4.tsx la meilleure version
...
const SortableItem: React.FC<SortableItemProps> = ({ id, item, index, onRemove, onMoveUp, onMoveDown, isFirst, isLast }) => {
    ...
    {/* Appel de la fonction getDetails pour afficher les informations spécifiques */}
    {/* La logique getDetails sera définie à l'intérieur du composant SortableItem */}
    {(() => {
        if (item.type === 'activity') {
            const objectifsDescriptions = item.objectifs?.map(obj => typeof obj === 'string' ? obj : obj.description_objectif).join(', ');
            return (
                <>
                    <div className="text-gray-500 text-xs mt-1"> <strong> Description: </strong> {item.description || 'Pas de description.'}</div>
                    {objectifsDescriptions && objectifsDescriptions.length > 0 && (
        ...
        if (item.type === 'evaluation') {
            const connaissancesTitles = item.connaissances?.map(c => typeof c === 'string' ? c : c.titre_connaissance).join(', ');
            const capacitesTitles = item.capacitesEvaluees?.map(ch => typeof ch === 'string' ? ch : ch.titre_capacite_habilete).join(', ');
            return (
                <>
                    <div className="text-gray-600 text-xs italic">{item.description || 'Pas de description.'}</div>
                    {item.type_evaluation && <div className="text-gray-500 text-xs mt-1">Type: {item.type_evaluation}</div>}
