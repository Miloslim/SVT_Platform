// src/components/planipeda/chapitreplanifier/ActivitySelector.tsx
...
// Il renvoie l'ID de l'activité sélectionnée et ses détails pour un affichage en lecture seule.
...
// Cette structure inclut désormais tous les champs nécessaires pour un PlanActivity
export interface ActivityDisplayData { // Exporté car utilisé par le parent
  id: number; // C'est l'ID de la table 'activites' (la source)
...
interface ActivitySelectorProps {
  // MODIFIÉ: onActivitySelected passe l'ID de l'activité source et des détails pour l'affichage initial
  onActivitySelected: (activitySourceId: number, activityDetails: Omit<ActivityDisplayData, 'id'>) => void;
  onCancel: () => void;
...
const ActivitySelector: React.FC<ActivitySelectorProps> = ({ onActivitySelected, onCancel, chapitreReferenceId, niveauId, optionId, uniteId }) => {
      ...
      try {
        let query = supabase.from('activites')
          .select(`
            ...
            titre_activite,
            description,
            duree_minutes,             -- NOTE: Votre table maîtresse a 'duree_minutes', pas 'duree_estimee_minutes'
            modalite_deroulement,      -- NOTE: Votre table maîtresse a 'modalite_deroulement', pas 'modalites'
            materiel,                  -- NOTE: Votre table maîtresse a 'materiel'
            ressource_urls,            -- NOTE: Votre table maîtresse a 'ressource_urls' (ARRAY), pas 'ressources' (TEXT)
        ...
        const formattedActivities: ActivityDisplayData[] = data.map((item: any) => {
          ...
          return {
            ...
            duree_estimee_minutes: item.duree_minutes, // Mappage de duree_minutes
            modalites: item.modalite_deroulement ?? "", // Mappage de modalite_deroulement
            ressources: (item.ressource_urls || []).join(', '), // Concaténation des URLs si array
  ...
  const handleInsertSelected = () => {
    if (selectedActivity) {
      // MODIFIÉ: Passe l'ID source et les autres détails pour l'affichage initial
      const { id, ...details } = selectedActivity;
      onActivitySelected(id, details);
    } else {
