//src/planipeda/chapitreplanifier/Sortabltem.tsx
// Assurez-vous que tous les imports nécessaires sont présents pour ce composant
...
import {
    ...
    FlaskConical // REMPLACÉ LayoutStack par FlaskConical pour séquence ou bloc général
...
const SortableItem: React.FC<SortableItemProps> = ({
    ...
    // Utilisé FlaskConical pour le cas par défaut (si ce n'est pas une activité ou une évaluation, probablement une séquence)
    const IconComponent = item.type === 'activity' ? Activity : (item.type === 'evaluation' ? BookText : FlaskConical);
        ...
        <tr ref={setNodeRef} style={style} className={`relative transition-transform duration-300 ease-in-out ${bgColor} border-b border-gray-200 hover:shadow-lg`}>
            ...
            <td className="px-2 py-3 whitespace-nowrap text-sm text-gray-700 text-center">
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold
                    ...
                    {item.type === 'activity' ? 'Activité' : (item.type === 'evaluation' ? 'Évaluation' : 'Séquence')} {/* Ajouté 'Séquence' */}
                </span>
            </td>
            ...
            <td className="px-4 py-3">
                <div className="flex items-center">
                    ...
                    <div className={`text-sm font-medium ${titleColor}`}>
                        ...
                        {item.type === 'evaluation' && (item as any).type_evaluation && ( // Cast to any for type_evaluation
                            <p className="text-gray-500 text-xs mt-1">
                                Type: {(item as any).type_evaluation}
                            </p>
                        )}
                    </div>
                </div>
