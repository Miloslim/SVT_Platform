...
// Chemin: src/components/planipeda/ScenarioEditor/SequenceFormchptr.tsx
...
import {
    SequenceFormData,
    SequenceItem,
    AddedActivityDetails, // Renommé de AddedActivityItem pour correspondre à vos types
    AddedEvaluationDetails, // Renommé de AddedEvaluationItem pour correspondre à vos types
    SequenceStatus,
...
const SequenceForm: React.FC<SequenceFormProps> = ({
    ...
    const [sequenceData, setSequenceData] = useState<SequenceFormData>(
        initialSequenceData || {
            ...
            duree_estimee: null,
            prerequis: null,
            // chapitre_id et ordre ne sont pas initialisés ici car ils viennent des props/sélection
        }
    );
     ...
     */
    const handleActivityCreated = useCallback((newActivity: AddedActivityDetails) => {
        ...
        // Vérifiez que newActivity.id est un nombre valide avant de l'utiliser
        if (typeof newActivity.id !== 'number' || isNaN(newActivity.id)) {
            console.error("ERREUR: ID d'activité invalide reçu de la modale:", newActivity.id);
            toast.error("Impossible d'ajouter l'activité: ID invalide.");
            setShowActivityEditor(false); // Ferme la modale même en cas d'erreur
            return;
        }
        ...
        setSequenceItems(prevItems => {
            ...
            const newItem: SequenceItem = {
                ...
                description: newActivity.description_activite || undefined, // Utilisez description_activite, conservez undefined si absente
     ...
     */
    const handleEvaluationCreated = useCallback((newEvaluation: AddedEvaluationDetails) => {
        ...
        // --- DÉBUT DE LA CORRECTION ---
        // Vérifiez que newEvaluation.id est un nombre valide avant de l'utiliser
        if (typeof newEvaluation.id !== 'number' || isNaN(newEvaluation.id)) {
            console.error("ERREUR: ID d'évaluation invalide reçu de la modale:", newEvaluation.id);
            toast.error("Impossible d'ajouter l'évaluation: ID invalide.");
            setShowEvaluationEditor(false); // Ferme la modale même en cas d'erreur
            return;
        }
        // --- FIN DE LA CORRECTION ---
    ...
    const handleRemoveSequenceItem = useCallback((type: 'activity' | 'evaluation', id: number) => {
        ...
        setSequenceItems(prevItems => {
            ...
            // Notifie le parent des changements pour la persistance
